...
class Anova(Filter):
    """
    Class that uses anova as a filter method
    """
    ...
    def start(self, pid, result_queue):
        ...
        top_k_indices = np.argsort(p_values)[:self.k]
        top_k_features = self.cols[top_k_indices]
        for i in range(len(self.model)):
            ...
            time_debut = timedelta(seconds=(time.time() - debut))
            print_out = self.print_(print_out=print_out, name="ANOV", maxi=score, best=s, mean=s,
