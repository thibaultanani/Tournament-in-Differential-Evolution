...
class MutualInformation(Filter):
    """
    Class that uses mutual information as a filter method
    """
    def __init__(self, name, target, model, train, test=None, drops=None, metric=None, Tmax=None, ratio=None,
        ...
        self.path = os.path.join(self.path, 'mutual' + self.suffix)
    ...
    def start(self, pid):
        ...
        top_k_features = self.cols[np.argsort(mi_values)[::-1][:self.k]]
        for i in range(len(self.model)):
            ...
            time_debut = timedelta(seconds=(time.time() - debut))
            print_out = self.print_(print_out=print_out, name="MI  ", maxi=score, best=s, mean=s,
